...
<p align=justify>The Actor network that parameterizes policies consists of three fully-connected layers of shape (33, 400), (400, 300), (300, 4), with Rectified Linear Unit non-linearities in-between, and a Tanh non-linearity to output a float within the [-1., 1.] range that this environment accepts for the action space. The Critic network that approximates the value function is defined by a sequence of three fully-connected layers of shape (33, 400), (404, 300), (300,1), with two ReLU layers in-between, and outputs the estimated mean value. Both networks parameterize the state space to understand the agents situation in the environment, which is why they take inputs of size 33, equal to the state dimension. Additionally, the critic network takes as input the action space in the second linear layer in order to calculate state-action values, which explains the difference between the output of the first layer (400) and the input of the second layer (404 = 400 + 4_action dim).</p>
