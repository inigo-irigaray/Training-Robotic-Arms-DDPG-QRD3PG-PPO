...
class PPOAgent:
    def __init__(self, obs_size, act_size, num_agents, hid1=400, hid2=300, norm='layer', actor_lr=1e-4, critic_lr=1e-3,
        ...
        self.critic_optim = optim.Adam(self.actor.parameters(), lr=critic_lr)
        self.gamma = gamma
    ...
    def step(self, obs, trajectory, explore=True):
        ...
        logstd = self.old_actor.logstd#.data.cpu().numpy()
        
        if explore:
            actions = mu + torch.exp(logstd) * torch.normal(mu, torch.exp(logstd))#mu + np.exp(logstd) * np.random.normal(size=logstd.shape)
            actions = torch.clamp(actions, -1, 1)#np.clip(actions, -1, 1)
            logprobs = self.logprobs(mu, logstd, actions)
        ...
        actions = torch.clamp(mu, -1, 1)#np.clip(mu, -1, 1)
        logprobs = self.logprobs(mu, logstd, actions)"""
    ...
    def evaluate(self, obs, actions):
        ...
        ###act_logprobs = self.logprobs(mu, self.actor.logstd, actions)
    ...
    def update(self, trajectory, writer=None, idx=None):
        ...
        #print('ref', adv_ref.shape)
        #print('ref', adv_ref)
        #print('adv', adv_est.shape)
        #print('adv', adv_est)
    ...
    @classmethod
    def init_from_env(cls, env_info, brain, hid1=400, hid2=300, norm='layer', actor_lr=1e-4,
                      critic_lr=1e-3, gamma=0.99, gae_lambda=0.95, epochs=1, eps=0.01):
