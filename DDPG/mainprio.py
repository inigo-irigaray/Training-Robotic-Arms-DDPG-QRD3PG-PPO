...
def run(config):
    ...
    print(ddpg.actor)
    print(ddpg.critic)
    repbuffer = buffer.PrioritizedReplayBuffer(capacity=config.capacity, num_agents=num_agents, alpha=config.alpha)
    episode = 0
    it = 0
    ...
    while True:
        ...
        while True:
            it += 1
            beta = min(1.0, config.beta + it * (1.0 - config.beta) / config.beta_frames)
